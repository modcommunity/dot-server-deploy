...
- **Parties, and a booking that holds.** A party on the TMC website can book a server for an evening; a **private** booking admits that party and nobody else (plus anybody with the `reservation` flag), and a **public** one holds a seat for every member still loading in. It chains onto the ban list rather than replacing it, it survives a `changelevel`, and an owner books their own server from the console with `party_reserve <partyId> <minutes> [private]`. With an integration token the server also reports who from which party is here, so the website records their play. Each game's chat gains a `party` channel — `/party_say hello` reaches your party and nobody else — and a signed-in player's client says which party it is in on its own. All of it is `cfg/party.yml`; `party_status` shows what is in force.
- **"Save that", and evidence with every ban.** Every server keeps the last minute of what happened in memory — who connected, who said what (a line a filter blocked included), which commands ran, who was kicked — capped at 32 MiB and in practice a few kilobytes. `replay save [seconds] [why]` at the console, over RCON or as `!replay save` in chat writes it to `data/replays/clips/` as a replay dot-replay's own reader verifies. A kick or a ban issued on the server saves a clip by itself and files its final hash with the punishment — on the ban record, in the audit log, and in a game's moderation record — so a disputed ban has something to be checked against that nobody can quietly edit. Writing every match to a file is one switch away and off. All of it is `cfg/replay.yml`; `replay` shows what is held.
- **Friends, in the client.** A player signed in through the website sees which friends are online and where, and a Join beside the ones that can be followed — into their party, or to a server this client has been on. Their own presence follows them: in a match, on the menu, gone when they quit. It is the website's friends list, so it switches itself off quietly on a site that does not serve the routes yet.
- **A matchmaking queue, when you want one.** Off by default. `cfg/matchmaking.yml` turns it on with playlists: ratings per queue, parties kept together, balanced sides and an accept step, through `/mm_queue <playlist>`, `/mm_accept`, `/mm_leave` and `mm_status`.
