...
mkdir -p "$STAGE/dot-server-setup-test"
if (cd "$ROOT" && git ls-files -z | tar --null -T - -cf -) \
    | tar -xf - -C "$STAGE/dot-server-setup-test"; then
...
for gone in addons game scenes; do
    if [ -e "$STAGE/dot-server-setup-test/$gone" ]; then
...
if (cd "$STAGE/dot-server-setup-test" \
...
mv "$STAGE/dot-server-setup-test" "$SHIP/tmc-server" || exit 1
rm -rf "$STAGE"
